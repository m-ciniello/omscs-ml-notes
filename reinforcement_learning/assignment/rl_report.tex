...
plausible mechanism: $n = 3$ returns bootstrap from $n$ actual rewards
instead of $1$, propagating reward signal $n \times$ faster and
directly cutting 1-step bootstrap variance --- the binding variance
source early in training when CartPole episodes terminate in 10--50
steps, so $n$-step is the only intervention in our ablation that
attacks it at the target. PER reweights which transitions are replayed (variance
...
means the weaker claim that ``components compose''
(Hessel et al.~\cite{hessel2018}) is \emph{not} supported at our scale.
H0 still prevails in the end: Rainbow-medium reaches $447$, above
