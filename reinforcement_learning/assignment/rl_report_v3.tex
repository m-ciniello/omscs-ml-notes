...
\textbf{Q-Learning} uses the max-target
...
behaviour policy. The practical consequence is that SARSA and Q-Learning diverge whenever exploration
is \emph{costly} near the optimal path (e.g.\ cliff-walking), where SARSA
accounts for the risk of random missteps and Q-Learning does not. Both
...
network, and optimizes the Huber loss with Adam~\cite{mnih2015}. Hidden
...
extra PE round plus third improvement step tips the balance back
to VI by $17$--$21\%$ at $\theta = 10^{-9}$. PE itself saturates
...
them. The residual sub-$0.005$ gap is a finite-sample floor, not
a structural one: tuned model-free \emph{approaches} DP on a
...
$(1,1,6,6) \to \mathbf{491}$, $(3,3,6,6) \to 392$,
...
sampling \emph{collapses} it to $84 / 68$.
...
\emph{common} VI/PI eval return ranges from $\sim$$67$ (fine grid,
low-$\varepsilon$ trained sampling) to $\sim$$491$ (coarsest grid,
...
sweeps peak near the defaults with shallow drop-off on either side;
...
The grid sweep is an inverted-U for both algorithms. Adding
cart-position bins at the coarsest pole resolution ($1 \to 3$)
lifts SARSA by $+64$ and Q-Learning by $+98$; further refining the
pole axis to $(3,3,8,12)$ adds only $+10 / +1$; and the finest grid
...
and shallower returns on either side --- Blackjack's
...
\begin{itemize}
\itemsep0em
\item baseline DQN: $156.9 \pm 39.4$,
\item + Double~\cite{vanhasselt2016}: $197.0 \pm 50.8$~~($\Delta = +40$),
\item + Dueling~\cite{wang2016}: $124.7 \pm 30.1$~~($\Delta = -32$),
\item + PER~\cite{schaul2016}: $213.7 \pm 70.2$~~($\Delta = +57$),
\item + $n$-step~\cite{sutton1988}: $441.7 \pm 47.5$~~($\Delta = +285$),
\item Rainbow-medium (all four): $447.1 \pm 56.4$~~($\Delta = +290$).
\end{itemize}
...
captures essentially all of Rainbow's gain
($+285$ vs.\ Rainbow-full's $+290$, within noise); PER adds $+57$;
Double adds $+40$; and Dueling \emph{actively regresses} ($-32$). A
plausible mechanism: $n = 3$ returns shorten the effective credit-assignment
horizon, which is the binding variance source early in training when
CartPole episodes terminate in 10--50 steps, so $n$-step is the only
intervention in our ablation that directly reduces 1-step bootstrap
variance. PER reweights which transitions are replayed (variance
reduction on the replay side but not the target); Double refines the
target's max (bias reduction but leaves bootstrap variance intact);
Dueling splits a two-action output into value and advantage heads,
which on a small MLP with well-correlated $Q$-values across actions
costs gradient signal without buying a variance-reduction benefit ---
hence the negative $\Delta$. Rainbow-full $\approx$ $n$-step alone
means the weak ``components compose'' claim
(Hessel et al.~\cite{hessel2018}) is \emph{not} supported at our scale.
H0 does still win in the end: Rainbow-medium reaches $447$, above
the tabular $\sim$$298$ ceiling --- but the representation benefit
only materializes once $n$-step returns are layered on.
...
narrowly) --- the envelopes bind at different grids, which is why
``best family'' changes with discretization.

\textbf{H1 (Blackjack DP).} Partially supported. Policies match
to four significant figures at every $\gamma$, as predicted. The
backup prediction is only right at high $\gamma$: VI cheaper than
PI by $17$--$21\%$ for $\gamma \geq 0.95$, but PI \emph{cheaper}
by $\sim 25\%$ for $\gamma \leq 0.9$. The driver is $K$
...
\textbf{H4 (discretization + $\gamma$).} Supported. Pole- and
cart-axis bins both contribute; finest grid regresses from
sample starvation. $\gamma = 0.99$ is CartPole's clear
optimum; Blackjack $\gamma$-invariance confirmed across five
values.
...
We run only a single default discretization for the CartPole tabular
comparison against DQN (the grid is ablated elsewhere but at matched
HPs). A more thorough study would tune HPs \emph{within} each grid,
since the optimal $\alpha$ / $\varepsilon$-decay likely depends on
cell count. Our DQN EC explicitly drops C51 and NoisyNets up front, so
we cannot claim the full Rainbow-paper result; our findings are
strictly a four-of-six Rainbow-medium ablation. Our Blackjack MDP is
hand-written against the Gymnasium \texttt{Blackjack-v1} rules; a
future check would be to sanity-check transition probabilities against
empirical Gym rollouts (see the TODO in the repo). Finally, the
trained-$\varepsilon$ sampling study (Theme A) uses
\emph{SARSA}-pretrained policies as the sampling source; we did not
rerun it with Q-Learning-pretrained policies. Given H3's finding that
SARSA and Q-Learning are indistinguishable on CartPole, we expect the
result to carry over, but we stop short of asserting it.
