...
AUUC and Uplift@k tell you whether the model *ranks* users well. But the allocation optimizer (see [Objective Design](../uplift_modeling/objective_design.md)) doesn't just use rankings — it uses the CATE *magnitudes* to solve the knapsack. If the magnitudes are wrong, the allocator makes bad decisions even if the ranking is perfect. This is where calibration comes in.
