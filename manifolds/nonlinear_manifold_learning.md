...
**Definition.** A **manifold** (specifically, a smooth $k$-dimensional manifold) is a topological space such that every point has a neighborhood **homeomorphic** to $\mathbb{R}^k$ — meaning there exists a continuous, invertible map (with continuous inverse) between that neighborhood and an open subset of $\mathbb{R}^k$. In practical terms: if you zoom in close enough to any point on the manifold, the local patch looks and behaves like ordinary $k$-dimensional flat space, even if the manifold is globally curved. You can lay a $k$-dimensional coordinate grid on any small patch without tearing or folding. $k$ is called the **intrinsic dimension** of the manifold. Examples:
