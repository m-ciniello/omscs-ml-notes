...
- A user with high predicted GMV is not necessarily a good reward target. They might spend $200 regardless of whether they receive a reward (User 1 in our [counterfactual examples](objective_design.md)). Giving them $10 generates $0 in incremental GMV.
...
The CATE answers: *"For a user with features $x$, how much does a $t reward increase their expected GMV?"* This is the object the uplift model estimates, and it is exactly the $\text{CATE}(u, t)$ that feeds into the allocation optimizer from the [objective design](objective_design.md).
